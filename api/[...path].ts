...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (seg[0] === "admin-upload-image" && seg.length === 1 && method === "POST") {
      ...
      const { uploadPortfolioImageFromDataUrl } = await import("../frontend/lib/api/uploadBlob.js");
    ...
    if (seg[0] === "admin-import-default-projects" && seg.length === 1) {
      ...
      if (!(await requireAdminJwt(req, res))) return;
      const { importMissingDefaultProjects } = await import("../frontend/lib/seedDefaultPortfolio.js");
